...
function engineStart()
{
	...
	mCanvas.style.backgroundColor = 'white'
...
var ColorGrey =  rgba(128, 128, 128, 1)
...
function Sprite(imageSrc)
{
	...
	this.x = 0
	this.y = 0
	this.width = function() { return this.frameSize.x * this.scale.x }
	this.height = function() {  return this.frameSize.y * this.scale.y }
	this.frameRow = 0
	this.frameColumn = 0
	...
	{
		return rect(	
			this.x + this.width() * this.collision.x, 
			this.y + this.height() * this.collision.y, 
...
/*
			...
			if(this.drawCollision)
			{
				...
				coll.x = Math.floor(coll.x - cameraPosition.x);
				coll.y = Math.floor(coll.y - cameraPosition.y);
